...
> Generated: 2026-04-21T03:28:06.879Z
...
└── CHANGELOG.md             # Version history```The`.claude-plugin/`directory contains the`plugin.json`file. All other directories (commands/, agents/, skills/, output-styles/, monitors/, hooks/) must be at the plugin root, not inside`.claude-plugin/`.
...
Disable a plugin without uninstalling it.```claude plugin disable <plugin> [options]```**Arguments:**
...
Follow semantic versioning for plugin releases:```{
  "name": "my-plugin",
  "version": "2.1.0"
}```**Version format**:`MAJOR.MINOR.PATCH`-   **MAJOR**: Breaking changes (incompatible API changes)
