...
if echo "$PROMPT" | grep -iqE '(npm audit|update.*packages|outdated.*packages|packages.*update|packages.*outdated|dependency.*update|update.*dependenc|security.*fix.*package|vulnerabilit.*package|maintain.*package|package.*maintain|unused.*dependenc|devDependenc|package.json.*(optimiz|aufräum|clean))'; then
