...
**Rule:** When your code changes cause existing (non-story) tests to fail, analyze each failure with the decision tree below: update the test for an intended breaking change, fix the code for an unintended side effect. This includes failures that predate the current changes or seem unrelated. A green test suite is a non-negotiable prerequisite for completing any work.
...
- **Every failing test gets fixed** — never ignore, skip, or defer one, even if it predates the current work
