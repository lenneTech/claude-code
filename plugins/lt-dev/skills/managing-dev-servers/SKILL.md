...
description: 'Rules for starting, monitoring, and stopping local dev servers (nuxt dev, nest start, pnpm run dev, pnpm build --watch, Playwright) and for closing the Chrome DevTools MCP browser afterward. Prefers `lt dev up/down/status/tunnel` for lt CLI projects: stable HTTPS URLs, project-scoped env vars, parallel projects without port or auth cross-wiring, and Cloudflare Quick Tunnel exposure. Falls back to the run_in_background / pkill contract for non-lt projects, so no orphaned process blocks the session. Activates whenever a long-running process starts for manual validation, Chrome DevTools MCP debugging, TDD, framework linking, or E2E, or when browser tabs must be closed after a walk.'
...
**Limit local Playwright runs to new + affected specs to keep TDD loops fast.** The full Playwright suite is slow and runs in **CI**. During local development / TDD, default to `lt dev test -- <spec>` (or `lt dev test -- tests/e2e/<file>.spec.ts`); the equivalent for non-lt-projects is `pnpm exec playwright test <spec>` (or `pnpm dlx playwright test <spec>` when Playwright is not installed locally). Backend Unit + API stay unrestricted — they're fast. Only run the **full** local Playwright suite when the user explicitly asks (or when an orchestrator like `production-ready` calls for it).
...
- **A green `lt dev test` does NOT prove the CI path — the two run different `NODE_ENV`s.** `lt dev test` starts the API with `NODE_ENV=local`; a GitLab pipeline starts it with `NODE_ENV=ci` (and Docker-compose stages use `develop`/`test`/`production`). Anything that differs per environment — cookie handling, `exposeTokenInBody`, seeds, cron jobs, fail-fast guards — is therefore **untested locally no matter how green the suite is**. This is not hypothetical: a project whose whole Playwright suite passed locally had every authenticated spec fail in CI, because `exposeTokenInBody` (set in `e2e`/`ci`, not in `local`) put a JWT in the session cookie and the session could never be restored. Two full pipeline cycles were spent before the environment difference was even suspected. **When a feature touches authentication, sessions, cookies, seeding or env-gated config, reproduce the CI stack locally before pushing:** start the built API with the pipeline's `NODE_ENV` and env vars, start the built app, and run Playwright against it. That is a few minutes; a red pipeline shard is thirty.
- **`lt dev test` expects the PROJECT to reset the test DB — it does not do it itself.** The CLI hands the isolated DB over as `MONGO_URI` and relies on the project's Playwright `globalSetup`/`globalTeardown` to empty it. A project without one accumulates data run over run; the failures then look like flaky tests but are leftovers (a spec opening "the Lieferschein type" finds one a run left behind weeks ago). Verify the hook exists. Prefer **teardown** over setup: the API seeds its initial administrator while booting, which happens BEFORE Playwright runs — dropping the database at that point deletes the account every spec signs in with, and the whole suite dies on the auth setup. Guard the drop on the database name (`endsWith('-test')`) so a misconfigured `MONGO_URI` can never wipe development data.
...
- **`pkill` reaches across sessions, and that is the failure nobody expects** — `pkill -f "nuxt dev"` kills every matching process belonging to your OS user, including the dev server a **parallel Claude Code session** started for its own work. The peer then watches its server die mid-walk with no cause it can see from inside its own session. `lt dev up`/`lt dev down` avoid this entirely (they own their processes per project), which is the strongest reason to prefer them in a machine that runs several sessions. For a non-lt project, record the PID at start and kill by PID. Before any broad `pkill`, `ListAgents` tells you whether a peer is even live; see the `coordinating-peer-sessions` skill for what to do when one is.
