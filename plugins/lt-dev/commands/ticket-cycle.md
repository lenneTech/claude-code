...
- **Ticket states are a claim protocol.** Every filed follow-up is created in **`Open` with a project assigned** (never `Backlog`, never project-less — the auto-pick pool excludes both, so either one loses the ticket for good). A follow-up that can only be worked **after this ticket is merged into the base branch** (`dev` / `development`) is created in **`Blocked`**, naming the blocking ticket in its description, and moved to `Open` once STEP 4b's healthy-dev-deploy verification confirms the merge is live — the auto-pick filter (STEP 1 Phase 1) admits every unassigned "Open" ticket, so leaving a dependent follow-up in `Open` gets it started against unmerged code by a parallel session. Do not carry it as prose-only: a ticket that exists solely in a summary is lost. **Never pre-set `In Progress`** on a ticket you merely intend to take next — it then reads as actively worked, and if anything intervenes nobody picks it up again. `In Progress` is set when work actually begins, after re-checking the ticket is still unclaimed (`take-ticket` STEP 3).
